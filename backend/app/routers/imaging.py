...
from app.schemas.imaging import ImagingInvoiceCreate, ImagingInvoiceResponse, ImagingItemDetail, ImagingPrescriptionPendingResponse
...
async def get_pending_prescriptions(
    ...
    if _role_value(current_user) not in ["caissier_imagerie", "admin"]:
    ...
    result: list[ImagingPrescriptionPendingResponse] = []
...
async def create_imaging_invoice(
    ...
    count = db.query(Facture).filter(Facture.numero_facture.like(f"FACT-IMG-{year}-%")).count()
    ...
    facture = Facture(
        ...
        commentaire="Facture imagerie depuis prescription transferee",
    )
    db.add(facture)
    db.flush()
